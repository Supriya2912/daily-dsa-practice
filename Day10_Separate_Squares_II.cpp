...
### Problem
Given multiple axis-aligned squares that may overlap, find the horizontal line that splits the total covered area into two equal halves.

### Approach
This problem is solved using a **sweep line algorithm** combined with a **segment tree**.

#### Key Steps
1. Compress X-coordinates to reduce range
2. Create sweep events for square top and bottom edges
3. Sweep from bottom to top along Y-axis
4. Maintain active X-interval coverage using a segment tree
5. Accumulate area between consecutive Y-levels
6. Find the Y-coordinate where accumulated area reaches half of total

### Data Structures Used
- Coordinate Compression
- Segment Tree (to maintain active X coverage)
...
### Key Insight
The area covered by active squares between two Y-coordinates is equal to:
